...
void loop()
{
  ...
  { // Entra no laço se comunicação está ok
    if (gps.encode(gpsSerial.read()))
    {                               // decodifiação de dados recebidos
      gps.encode(gpsSerial.read()); // processar dados brutos
      Serial.println("Processando/aguardando dados GPS");
      while (lat == 0 && lon == 0)
      {                               // loop para aguardar latitude e longitude
        gps.encode(gpsSerial.read()); // confere se dados brutos chegaram e processa
        read_all_data_gps();          // lê todos os dados
        if (millis() - time_control >= time_comp)
        { // condição para imprimir status (aguardando)
          time_control = millis();
          Serial.print(".");
          if ((millis() - now) >= time_wait_gps)
          {                                                  // Se não encontrar lat e lon os dados são enviados sem esses
...
wait_confirmation:
  ...
  { // laco para receber confirmação
    ...
    { // em n segundos, se não chegou confirmacao
    ...
    if (millis() - time_geral >= time_finish_resend)
    { // fim do laço de tentativas
    ...
    {                                            // se chegou algum dado
      ...
      if (strcmp(conf, "OK") == 0)
      {                                        // Se a confirmação chegou for a mesma que a esperada
        ...
        { // se a qualidade do sinal estiver boa
...
void toggleSerial_lora(bool enable)
{ // funcao ligar/desligar comunicao com LORA
  if (enable)
  {
